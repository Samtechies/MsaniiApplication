package com.example.msaniiapplication
...
import com.example.msaniiapplication.databinding.ActivityRegisterBinding
...
//  Click
    ...
    private fun showNameExistAlert(isNotValid: Boolean) {
        binding.etFullname.error = if (isNotValid) "Enter Full Name" else null
    }

    private fun showTextMinimalAlert(isNotValid: Boolean, text:String){
        if(text == "Username")
            binding.etUsername.error = if (isNotValid) "$text User Name should be 6 characters or more" else null
        else if (text == "Password")
            binding.etPassword.error = if (isNotValid) "$text Password enter 8 characters or more" else null
    }

    private fun showEmailValidAlert(isNotValid: Boolean) {
        binding.etEmail.error = if (isNotValid) "Enter your Email!" else null
    }

    private fun showPasswordConfirmAlert(isNotValid: Boolean) {
        binding.etConfirmPassword.error = if (isNotValid) "Password is incorrect" else null
    }
